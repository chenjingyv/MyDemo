...
public class DemoImageCodeGenerator implements ValidateCodeGenerator {
    ...
    public ImageCode generator(ServletWebRequest request) {
        ...
        for(int i = 0; i < length; i++){
            ...
            affine.setToRotation(Math.PI / 4 * rand.nextDouble() * (rand.nextBoolean() ? 1 : -1), (w / length) * i + fontSize/2, h/2);
            g2.setTransform(affine);
            g2.drawChars(chars, i, 1, ((w-10) / length) * i + 5, h/2 + fontSize/2 - 10);
        }

        g2.dispose();
        return new ImageCode(code, image,  securityProperties.getCode().getImage().getExpireIn());
    }
}